...
{
	private int THE_SAME_PARTS_LIMIT = 30;
	...
	public void StartCutting(Order order)
	{
		...
		Write_Sheets_to_Order_LENGTH_CUT(order, SH_T_F_T_F, CP_T_F_T_F);
	}

	public CSheet GetCSheet_LENGTH_CUT(List<CPart> parts, int ListLength, int ListWidth, int Blade, int Padding, bool DoublePadding, LW16 PARAMS, double PSQ, double PPSQ, out double PPSQ_OUT)
	{
		L_L = ListLength;
		L_W = ListWidth;
		P = Padding;
		...
		List<CLine> cLines_LENGTH_CUT = GetCLines_LENGTH_CUT(parts, num2, num3, PARAMS);
		int num4 = num3;
		int num5 = 0;
			...
			for (int k = 0; k < cSheet.Lines.Count - 1; k++)
			{
				for (int l = k + 1; l < cSheet.Lines.Count; l++)
				{
					...
					if (cSheet.Lines_index[k] != array2[0] || cSheet.Lines_index[l] != array2[1] || array2[2] != -1)
					{
						...
						if (num13 >= num11 && (long)(num14 - num12) >= 0 && ((long)(num14 - num10) > 0 || ((long)(num14 - num10) == 0L && num13 - (cSheet.Lines[num8].W + cSheet.Lines[num9].W) > 0)))
						{
							num8 = k;
							num9 = l;
							array = array2;
		...
		{
			Continue_Line_LENGTH_CUT(cSheet.Lines[num18], parts, PARAMS);
		...
		while (!flag)
		{
			if (FastFindFirstPart(parts, cSheet.Remain.L, cSheet.Remain.W))
			{
				...
				int num19 = Find_WIDTH_part(parts, cSheet.Remain.L, cSheet.Remain.W, TURN_ON: true);
				...
				Place_Part_to_Line(cLine, parts, num19, io, _rez: false);
				Continue_Line_LENGTH_CUT(cLine, parts, PARAMS);
	...
	private void Continue_Line_LENGTH_CUT(CLine LINE, List<CPart> parts, LW16 PARAMS)
	{
		int num = -1;
		for (int num2 = FindSmallSnip(LINE.Snips, parts); num2 >= 0; num2 = FindSmallSnip(LINE.Snips, parts))
		{
			...
			num = ((!PARAMS.MAX_SQ) ? Find_WIDTH_part(parts, l, w, TURN_ON: true) : FindMaxSqPart(parts, l, w));
			if (num != -1)
			{
				if (PARAMS.OPTI_ON)
				{
	...
	private List<CLine> GetCLines_LENGTH_CUT(List<CPart> parts, int LL, int LW, LW16 PARAMS)
	{
		...
		while (!flag && num2 < LINES_LIMIT)
		{
			...
			CLine cLine = null;
			CLine cLine2 = null;
			num = ((!PARAMS.MAX_SQ) ? Find_WIDTH_part(parts, num3, num4, TURN_ON: false) : FindMaxSqPart(parts, LL, LW));
			if (num != -1)
			{
				cLine = MakeLine_LENGTH_CUT(parts, num, num3, num4, PARAMS, out var _);
				if (PARAMS.TURN_ON)
				{
					int index = num;
					if (num < -1)
					...
					{
						cLine2 = MakeLine_LENGTH_CUT(parts, num * -1 - 2, num3, num4, PARAMS, out var _);
					}
	...
	private CLine MakeLine_LENGTH_CUT(List<CPart> parts, int startPart, int LineLength, int LineWidth, LW16 PARAMS, out CLine PreCut)
	{
		CLine cLine = new CLine();
		CSnip cSnip = new CSnip();
		cLine.Snips = new List<CSnip>();
		...
		if (PARAMS.SAME_MAX)
		{
			int Min_L;
			int Total_Length;
			List<int> fixWidth = Get_Parts_with_FixWidth(parts, WD, cLine.W, PARAMS.TURN_ON, out Min_L, out Total_Length);
		...
		else
		{
			...
			if (io != -1)
			{
				while (!flag)
				{
					...
					num = Find_THE_SAME_WIDTH_part(parts, l, w, PARAMS.TURN_ON);
					if (num != -1)
					{
	...
	private int Find_WIDTH_part(List<CPart> parts, int LO, int WO, bool TURN_ON)
	{
		int result = -1;
		int num = 0;
		double num2 = 0.0;
		for (int i = 0; i < parts.Count; i++)
		{
			...
			else if (parts[i].Turn)
			{
				...
				{
					num3 = (TURN_ON ? ((parts[i].L < parts[i].W) ? parts[i].W : parts[i].L) : ((parts[i].L < parts[i].W) ? parts[i].L : parts[i].W));
				}
	...
	private int Find_THE_SAME_WIDTH_part(List<CPart> parts, int LO, int WO, bool TURN_ON)
	{
		int result = -1;
		double num = 0.0;
		for (int i = 0; i < parts.Count; i++)
		{
			...
			if (!TURN_ON)
			{
				...
				else
				{
					...
					if (parts[i].W > parts[i].L && LO >= parts[i].W && WO == parts[i].L)
					{
					...
					else if (parts[i].L > parts[i].W && LO >= parts[i].L && WO == parts[i].W && (long)(parts[i].Sq - num) > 0)
					{
						num = parts[i].Sq;
						result = i;
					}
			...
			for (int k = i + 1; k < Lines.Count; k++)
			{
				...
				for (int m = k + 1; m < Lines.Count; m++)
				{
					...
					else
					{
						...
						if ((long)(Lines[i].Parts_Sq + Lines[k].Parts_Sq + Lines[m].Parts_Sq - num4) >= 0)
						{
							array[0] = i;
							array[1] = k;
							array[2] = m;
	...
	private void Write_Sheets_to_Order_LENGTH_CUT(Order o, List<CSheet> Ss, List<CPart> CPARTS)
	{
		for (int i = 0; i < Ss.Count; i++)
		{
			...
			while (Ss[i].Lines.Count > 0)
			{
				...
				{
					if (Ss[i].Lines[j].W > num2 || (Ss[i].Lines[j].W == num2 && (long)(Ss[i].Lines[j].Parts_Sq - Ss[i].Lines[index].Parts_Sq) > 0))
					{
						index = j;
						num2 = Ss[i].Lines[j].W;
					}
	...
	private List<int> Get_Parts_with_FixWidth(List<CPart> parts, int W, int min_in, bool TURN_ON, out int Min_L, out int Total_Length)
	{
		...
		for (int i = 0; i < parts.Count; i++)
		{
			...
			{
				for (int l = 0; l < cPart.Qty - cPart.Plased; l++)
				{
					list.Add(i);
					Total_Length += cPart.L;
				}
	...
	private List<int> GetStartParts_for_Line_LENGTH_CUT(List<CPart> parts, List<int> FixWidth, int LineLength, int Minimal_L)
	{
		...
		while (!flag && num2 < PARTS_SORT_LIMIT)
		{
			...
			for (int j = 0; j < list.Count - 1; j++)
			{
				for (int k = j + 1; k < list.Count; k++)
				{
					FixWidth.Add(list[j]);
					FixWidth.Add(list[k]);
					int num8 = ((list[j] <= -1) ? parts[list[j] * -1 - 2].W : parts[list[j]].L);
					int num9 = ((list[k] <= -1) ? parts[list[k] * -1 - 2].W : parts[list[k]].L);
					int wO = num + B + num8 + B + num9;
					int[] array2 = Find_Zamena_PARTS_LENGTH_CUT(FixWidth, parts, wO, num8 + num9, Minimal_L, out check);
					if (check)
					{
						int num10 = 0;
						for (int l = 0; l < array2.Length; l++)
						{
							if (array2[l] != -1)
							{
								Get_ID_LD_WD(parts, FixWidth[array2[l]], out var _, out var LD2, out var _);
								num10 = num10 + B + LD2;
							}
						}
						if (num10 > num3)
			...
			if (num4 != -1 && num5 != -1)
			{
				...
				for (int m = 0; m < array.Length; m++)
				{
					if (array[m] != -1)
					{
						...
						Get_ID_LD_WD(parts, FixWidth[array[m]], out var _, out var LD3, out var _);
						num = num - B - LD3;
					}
				}
				int num11 = 0;
				for (int n = 0; n < array.Length; n++)
				{
					if (array[n] != -1)
					{
						FixWidth.RemoveAt(array[n] - num11);
						num11++;
					}
				}
			}
		...
		for (int num12 = 0; num12 < list.Count - 1; num12++)
		{
			...
			{
				Get_ID_LD_WD(parts, list[num12], out var _, out var LD4, out var _);
				Get_ID_LD_WD(parts, list[num13], out var _, out var LD5, out var _);
				if (LD5 > LD4)
				{
	...
	private void DEBUG_CR(CutTimeResults cr)
	{
	}
}
