...
public class Width_Alg
{
	...
	public void StartCutting(Order order)
	{
		...
		_ = DateTime.Now.TimeOfDay.Milliseconds;
		...
		while (!flag)
		{
			...
			{
				CSheet cSheet = GetCSheet_WIDTH_CUT(CPARTS, L_L, L_W, B, P, DoublePadding: true, Opti_ON: true, CleanParts: false);
				for (int i = 0; i < cSheet.Lines.Count; i++)
				{
					SET_OFF_Parts_in_Line(CPARTS, cSheet.Lines[i]);
				}
				CSheet cSheet_WIDTH_CUT = GetCSheet_WIDTH_CUT(CPARTS, L_L, L_W, B, P, DoublePadding: true, Opti_ON: false, CleanParts: false);
				if ((int)(cSheet_WIDTH_CUT.Parts_Sq - cSheet.Parts_Sq) > 0)
				{
					cSheet = cSheet_WIDTH_CUT;
				}
				else
				{
					for (int j = 0; j < cSheet_WIDTH_CUT.Lines.Count; j++)
					{
						SET_OFF_Parts_in_Line(CPARTS, cSheet_WIDTH_CUT.Lines[j]);
					}
		...
		Write_Sheets_to_Order_WIDTHCUT(order, SHEETS);
	}
	...
	public CSheet GetCSheet_WIDTH_CUT(List<CPart> parts, int ListLength, int ListWidth, int Blade, int Padding, bool DoublePadding, bool Opti_ON, bool CleanParts)
	{
		L_L = ListLength;
		L_W = ListWidth;
		P = Padding;
		...
		List<CLine> cLines_WIDTH_CUT = GetCLines_WIDTH_CUT(parts, num2, num3, Opti_ON);
		int num4 = num2;
			...
			for (int k = 0; k < cSheet.Lines.Count - 1; k++)
			{
				for (int l = k + 1; l < cSheet.Lines.Count; l++)
				{
					...
					if (cSheet.Lines_index[k] != array2[0] || cSheet.Lines_index[l] != array2[1] || array2[2] != -1)
					{
						...
						if (num12 >= num10 && (int)(num13 - num11) >= 0 && ((int)(num13 - num9) > 0 || ((int)(num13 - num9) == 0 && num12 - cSheet.Lines[num7].L - cSheet.Lines[num8].L > 0)))
						{
							num7 = k;
							num8 = l;
							array = array2;
		...
		{
			Continue_Line_WIDTH_CUT(cSheet.Lines[num17], parts, maxWay, optiON);
		...
		while (!flag)
		{
			if (FastFindFirstPart(parts, cSheet.Remain.L, cSheet.Remain.W))
			{
				...
				Continue_Line_WIDTH_CUT(cLine, parts, maxWay, OptiON: true);
	...
	private List<CLine> GetCLines_WIDTH_CUT(List<CPart> parts, int LL, int LW, bool OptiON)
	{
		...
		while (!flag && num2 < LINES_LIMIT)
		{
			...
			CLine cLine4 = null;
			CLine PreCut4 = null;
			num = Find_LENGTH_part(parts, num3, num4, Max_L: false);
			if (num != -1)
			{
				cLine = MakeLine_WIDTH_CUT(parts, num, num3, num4, THE_SAME_MAX: true, MaxWay: false, OptiON, out PreCut);
				SET_ON_Parts_in_Line(parts, PreCut);
				Continue_Line_WIDTH_CUT(PreCut, parts, MaxWay: true, OptiON);
				SET_OFF_Parts_in_Line(parts, PreCut);
				if (cLine != null && PreCut != null && (int)(PreCut.Parts_Sq - cLine.Parts_Sq) > 0)
				{
					cLine = PreCut;
				}
				cLine2 = MakeLine_WIDTH_CUT(parts, num, num3, num4, THE_SAME_MAX: false, MaxWay: false, OptiON, out PreCut2);
				SET_ON_Parts_in_Line(parts, PreCut2);
				Continue_Line_WIDTH_CUT(PreCut2, parts, MaxWay: true, OptiON);
				...
				if (parts[index].Turn)
				{
					...
					{
						cLine3 = MakeLine_WIDTH_CUT(parts, num * -1 - 2, num3, num4, THE_SAME_MAX: true, MaxWay: false, OptiON, out PreCut3);
						SET_ON_Parts_in_Line(parts, PreCut3);
						Continue_Line_WIDTH_CUT(PreCut3, parts, MaxWay: true, OptiON);
						SET_OFF_Parts_in_Line(parts, PreCut3);
						cLine4 = MakeLine_WIDTH_CUT(parts, num * -1 - 2, num3, num4, THE_SAME_MAX: false, MaxWay: false, OptiON, out PreCut4);
						SET_ON_Parts_in_Line(parts, PreCut4);
						Continue_Line_WIDTH_CUT(PreCut4, parts, MaxWay: true, OptiON);
	...
	private CLine MakeLine_WIDTH_CUT(List<CPart> parts, int startPart, int LL, int LW, bool THE_SAME_MAX, bool MaxWay, bool OptiOn, out CLine PreCut)
	{
		CLine cLine = new CLine();
		CSnip cSnip = new CSnip();
		cLine.Snips = new List<CSnip>();
		...
		if (THE_SAME_MAX)
		{
			int Min_W;
			int Total_Length;
			List<int> fixLength = Get_Parts_with_FixLength(parts, LD, cLine.W, cLine.W, out Min_W, out Total_Length);
		...
		else
		{
			...
			if (io != -1)
			{
				while (!flag)
				{
					...
					num = Find_THE_SAME_LENGTH_part(parts, l, w);
					if (num != -1)
					{
		...
		Continue_Line_WIDTH_CUT(cLine, parts, MaxWay, OptiOn);
		SET_OFF_Parts_in_Line(parts, cLine);
		return cLine;
	}

	private void Continue_Line_WIDTH_CUT(CLine LINE, List<CPart> parts, bool MaxWay, bool OptiON)
	{
		int num = -1;
		for (int num2 = FindSmallSnip(LINE.Snips, parts); num2 >= 0; num2 = FindSmallSnip(LINE.Snips, parts))
		{
			...
			num = ((!MaxWay) ? Find_LENGTH_part(parts, l, w, Max_L: true) : FindMaxSqPart(parts, l, w));
			if (num != -1)
			{
				if (OptiON)
				{
					int[] array = Check_part_for_last_in_Line(parts, l, w, rez: true, num);
	...
	private int Find_THE_SAME_LENGTH_part(List<CPart> parts, int LO, int WO)
	{
		int result = -1;
		double num = 0.0;
		for (int i = 0; i < parts.Count; i++)
		{
			...
			else if (parts[i].Turn)
			{
				...
				if (num2 > -1 && (long)(parts[i].Sq - num) > 0)
				{
					num = parts[i].Sq;
					result = ((parts[i].L != num2) ? (-1 * i - 2) : i);
				}
			}
		}
		return result;
	...
	private void Place_2_Parts_to_Line(CLine line, List<CPart> parts, int[] _2parts, int io)
	{
		...
		if (num4 != -1)
		{
			...
			if (flag && flag2)
			{
				num27 = (((long)((num14 - num13) * 10.0) == 0L) ? ((l < w) ? (num11 * -1) : num12) : (((long)((num14 - num13) * 10.0) >= 0) ? (num11 * -1) : num12));
			}
			else
			{
				if (flag)
			...
			for (int k = i + 1; k < Lines.Count; k++)
			{
				...
				for (int n = k + 1; n < Lines.Count; n++)
				{
					...
					else
					{
						...
						if ((int)(Lines[i].Parts_Sq + Lines[k].Parts_Sq + Lines[n].Parts_Sq - num4) >= 0)
						{
							array[0] = i;
							array[1] = k;
							array[2] = n;
	...
	private List<CPart> ConvertParts(List<Part> P)
	{
		PartsCount = 0;
		...
		for (int i = 0; i < P.Count; i++)
		{
			if (P[i].nPlased < P[i].Amount)
			{
				...
				cPart.L = P[i].Length_mm;
				cPart.W = P[i].Width_mm;
				cPart.Qty = P[i].Amount - P[i].nPlased;
				cPart.iD_in_Order = i;
				cPart.Turn = P[i].Turn;
				cPart.Plased = 0;
				PartsCount += cPart.Qty;
				list.Add(cPart);
			}
		}
		return list;
	...
	private void Write_Sheets_to_Order_WIDTHCUT(Order o, List<CSheet> Ss)
	{
		for (int i = 0; i < Ss.Count; i++)
		{
			...
			while (Ss[i].Lines.Count > 0)
			{
				...
				{
					if (Ss[i].Lines[j].L > num2 || (Ss[i].Lines[j].L == num2 && (int)(Ss[i].Lines[j].Parts_Sq - Ss[i].Lines[index].Parts_Sq) > 0))
					{
						index = j;
						num2 = Ss[i].Lines[j].L;
					}
				...
				for (int k = 0; k < Ss[i].Lines[index].PartIDs.Count; k++)
				{
					...
					part.Coords[nPlased].X = (num + crd.X) / 10;
					part.Coords[nPlased].Y = (p + crd.Y) / 10;
				...
				for (int l = 0; l < Ss[i].Lines[index].Snips.Count; l++)
				{
					...
					snip.nlist = -1;
					snip.Amount = 1;
					snip.X = (num + cSnip.CRD.X) / 10;
					snip.Y = (p + cSnip.CRD.Y) / 10;
					o.NSnips.Add(snip);
				}
	...
	private List<int> Get_Parts_with_FixLength(List<CPart> parts, int L, int W, int min_in, out int Min_W, out int Total_Length)
	{
	...
	private List<int> GetStartParts_for_Line_WIDTH_CUT(List<CPart> parts, List<int> FixLength, int LineWidth, int Minimal_W)
	{
		...
		while (!flag && num2 < PARTS_SORT_LIMIT)
		{
			...
			for (int j = 0; j < list.Count - 1; j++)
			{
				for (int k = j + 1; k < list.Count; k++)
				{
					...
					Get_ID_LD_WD(parts, list[j], out var _, out var _, out var WD2);
					Get_ID_LD_WD(parts, list[k], out var _, out var _, out var WD3);
					int wO = num + B + WD2 + B + WD3;
					int[] array2 = Find_Zamena_PARTS_WIDTH_CUT(FixLength, parts, wO, WD2 + WD3, Minimal_W, out check);
					if (check)
					{
						int num8 = 0;
						for (int l = 0; l < array2.Length; l++)
						{
							if (array2[l] != -1)
							{
								Get_ID_LD_WD(parts, FixLength[array2[l]], out var _, out var _, out var WD4);
								num8 = num8 + B + WD4;
							}
						}
						if (num8 > num3)
			...
			if (num4 != -1 && num5 != -1)
			{
				...
				for (int m = 0; m < array.Length; m++)
				{
					if (array[m] != -1)
					{
						...
						Get_ID_LD_WD(parts, FixLength[array[m]], out var _, out var _, out var WD5);
						num = num - B - WD5;
					}
				}
				int num9 = 0;
				for (int n = 0; n < array.Length; n++)
				{
					if (array[n] != -1)
					{
						FixLength.RemoveAt(array[n] - num9);
						num9++;
					}
				}
			}
		...
		for (int num10 = 0; num10 < list.Count - 1; num10++)
		{
			...
			{
				Get_ID_LD_WD(parts, list[num10], out var _, out var _, out var WD6);
				Get_ID_LD_WD(parts, list[num11], out var _, out var _, out var WD7);
				if (WD7 > WD6)
				{
